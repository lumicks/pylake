...
* Added feature to adaptively determine the fitting range when fitting power spectra using [`calibrate_force()`](https://lumicks-pylake.readthedocs.io/en/v1.8.0/_api/lumicks.pylake.calibrate_force.html#lumicks.pylake.calibrate_force) and [`fit_power_spectrum()`](https://lumicks-pylake.readthedocs.io/en/stable/_api/lumicks.pylake.fit_power_spectrum.html). Provide the parameter `corner_frequency_factor` to use this feature. When provided, the fitting range will be iteratively determined based on the corner frequency of the power spectrum.
...
* Added [voltage](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibration.ForceCalibrationItem.html#lumicks.pylake.calibration.ForceCalibrationItem.voltage), [sum_voltage](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibration.ForceCalibrationItem.html#lumicks.pylake.calibration.ForceCalibrationItem.sum_voltage) and [driving](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibration.ForceCalibrationItem.html#lumicks.pylake.calibration.ForceCalibrationItem.driving) properties which return raw calibration data. Note that these are only available when the option to export the raw data has been explicitly selected in Bluelake. When unavailable, these properties will return empty slices.
* Added [ForceCalibrationItem.recalibrate_with()](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibration.ForceCalibrationItem.html#lumicks.pylake.calibration.ForceCalibrationItem.recalibrate_with) and [ForceCalibrationItem.plot()](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibration.ForceCalibrationItem.html#lumicks.pylake.calibration.ForceCalibrationItem.plot) to recalibrate and/or plot calibrations performed in Bluelake. Note that the raw data needs to have been exported inside the item (Bluelake 2.7.x feature) for this feature to work.
* Added [`DwelltimeModel.err_amplitudes`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.DwelltimeModel.html#lumicks.pylake.DwelltimeModel.err_amplitudes) and [`DwelltimeModel.err_lifetimes`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.DwelltimeModel.html#lumicks.pylake.DwelltimeModel.err_lifetimes) to get asymptotic standard error estimates for the amplitudes and lifetimes. See the [`tutorial`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/tutorial/population_dynamics.html#standard-errors) for more information.
* Added the optional argument `with_stderr` to [`DwelltimeProfiles.plot()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.population.dwelltime.DwelltimeProfiles.html#lumicks.pylake.population.dwelltime.DwelltimeProfiles.plot) to plot quadratic approximation based on the asymptotic standard errors.
* Added option to save TIFFs bigger than 4 GB using `bigtiff=True` in [`ImageStack.export_tiff()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.ImageStack.html#lumicks.pylake.ImageStack.export_tiff) and [`Scan.export_tiff()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.scan.Scan.html#lumicks.pylake.scan.Scan.export_tiff).
* Added [`PowerSpectrum.compress()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.force_calibration.power_spectrum.PowerSpectrum.html#lumicks.pylake.force_calibration.power_spectrum.PowerSpectrum.compress#) to discard high frequency spectral data for a downsampled spectrum to conserve memory.
* Added `driving_sample_rate` parameter to [`calibrate_force()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.calibrate_force.html#lumicks.pylake.calibrate_force) and [`ActiveCalibrationModel`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.ActiveCalibrationModel.html#activecalibrationmodel) to specify the sample rate of the driving signal in active calibration.
* Added `driving_sample_rate` property to force calibration items to extract the sample rate of the driving input used to calibrate.
* Added `num_processes` parameter to [`DwelltimeModel.calculate_bootstrap()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.DwelltimeModel.html#lumicks.pylake.DwelltimeModel.calculate_bootstrap) to calculate bootstraps in parallel.

#### Improvements

* Force `num_points_per_block` to be integer when down-sampling a [`PowerSpectrum`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.force_calibration.power_spectrum.PowerSpectrum.html).
* [`KymoTrackGroup.ensemble_diffusion("cve")`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.kymotracker.kymotrack.KymoTrackGroup.html#lumicks.pylake.kymotracker.kymotrack.KymoTrackGroup.ensemble_diffusion) now raises a descriptive `ValueError` when invoked on a [`KymoTrackGroup`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.kymotracker.kymotrack.KymoTrackGroup.html) which doesn't have any tracks available which can be used to estimate a diffusion constant.

#### Bug fixes

* Fixed a bug in [`Kymo.plot_with_channels()`](https://lumicks-pylake.readthedocs.io/en/v1.7.0/_api/lumicks.pylake.kymo.Kymo.html#lumicks.pylake.kymo.Kymo.plot_with_channels) that would not correctly align the timeline channels in some cases. Prior to the fix, it assumed that slicing the channel with the kymograph start and stop time results in the origin being at time zero, but this may not be the case if the channel is sparsely sampled and the first point is much later (such as is the case for diagnostic channels).
