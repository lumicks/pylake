...
from .detail.caching import LazyCache
...
class Continuous:
    ...
    def from_dataset(dset, y_label="y", calibration=None):
        ...
        return Slice(
            Continuous(LazyCache.from_h5py_dset(dset) if caching.global_cache else dset, start, dt),
...
class TimeTags:
    ...
    def from_dataset(dset, y_label="y"):
        return Slice(
            TimeTags(LazyCache.from_h5py_dset(dset) if caching.global_cache else dset),
            labels={"title": dset.name.strip("/"), "y": y_label},
        )
