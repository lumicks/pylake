...
def _fit_power_spectra(
    ...
    if prior:
        scaled_model = ScaledModel(lambda f, *params: np.hstack((1 / model(f[:-1], *params), prior(params))), initial_params)
    else:
    ...
    if prior:
        ...
        sigma = np.hstack((sigma, 1e-6))
        data = np.hstack((data, 0*4e-7))
    ...
    return CalibrationResults(
        ...
        results={
            **model.calibration_results(
            ...
            "offset": solution_params[2] if model.has_offset else 0,
