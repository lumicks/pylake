...
@pytest.mark.parametrize(
    "sine_waves, expected_amp, expected_freq, expected_amp_std, guess, f_search, n_fit",
    [
        [[[0.5, 36.9], [1, 50]], 0.5, 36.9, 0.002256761211793501, 36, 5, 2],
        [[[0.34, 36.7], [1, 50]], 0.34, 36.7, 0.00225676105637345, 36, 5, 2],
        [[[0.45, 36.554], [1, 50]], 0.45, 36.554, 0.0022570399525330964, 36, 5, 2],
        # Does window exclude the bigger peak?
        [[[0.5, 36.9], [1, 42.0]], 0.5, 36.9, 0.0022567612246763924, 36, 5, 2],
        # Bigger window includes second peak
        [[[0.5, 36.9], [1, 42.0]], 1.0, 42.0, 0.0011283827921561406, 36, 10, 2],
        # Does it pick up on the second peak?
        [[[0.5, 36.9], [1, 41.0]], 1.0, 41.0, 0.0011283828084834118, 36, 6, 2],
        # Does it pick up on the second peak?
        [[[0.5, 36.9], [-1, 41.0]], 1.0, 41.0, 0.0011283827150010855, 36, 6, 2],
        # Fit range has both peaks
        [[[0.5, 36.9], [1, 42.0]], 0.617576, 41.999864, 0.0021032724481199858, 36, 10, 10],
        # Does it reject offset correctly?
        [[[0.5, 2.4], [0, 50]], 0.5, 2.4, 1.1561171403241089e-06, 3, 5, 2],
    ],
)
def test_driving_input_estimation(
    sine_waves, expected_amp, expected_freq, expected_amp_std, guess, f_search, n_fit
):
    sample_rate = 78125
    duration = 10
    ...
    amp, freq, amp_std = estimate_driving_input_parameters(
        sample_rate,
        y,
        guess,
        window_factor=10,
    ...
    np.testing.assert_allclose(amp_std, expected_amp_std, rtol=1e-6)
