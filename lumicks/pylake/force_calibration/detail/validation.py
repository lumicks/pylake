...
from lumicks.pylake import fit_power_spectrum
...
def check_corner_frequency(calibration):
    """Check whether the corner frequency is below the spectral resolution or fit range
...
def check_blocking_error(calibration, blocking_threshold):
    ...
    def upsample_frequency_axis(ps):
        ...
        upsampled_freq = (
            ps.frequency[:, np.newaxis] + (np.arange(-num / 2, num / 2) + 0.5) * df / num
        )
...
def compare_to_robust_fitting(calibration, factor):
    """Compare the obtained results to robust fitting.

    Given correct frequency exclusion ranges and a low enough noise floor, the difference between
    robust en regular fitting should be small. This tests verifies whether that is the case.
    ...
    calibration : lk.force_calibration.power_spectrum_calibration.CalibrationResults
        Power spectrum calibration result.
    factor : float
        Factor which determines how large the difference between robust and regular fitting is
        allowed to be (difference <= factor * ref_parameter).
    """
    robust = fit_power_spectrum(
        calibration.ps_data, calibration.model, bias_correction=False, loss_function="lorentzian"
    )

    param_deviation = ""
    parameters = ["Rf", "kappa", "Rd"]
    for param in parameters:
        ref_value, robust_value = (c[param].value for c in (calibration, robust))
        rel_diff = abs(robust_value - ref_value) / ref_value

        if rel_diff > factor:
            param_deviation += (
                f"{param}: regular: {ref_value:.3g}, robust: {robust_value:.3g}, "
                f"rel_diff: {rel_diff:.0%}\n"
            )

    if param_deviation:
        return (
            f"More than {factor:.0%} difference between robust and regular fit in parameter "
            f"estimates:\n{param_deviation}"
        )
...
def validate_results(
    calibration,
    alpha=0.1,
    desired_backing=1e-12,
    blocking_threshold=3,
    robust_difference=0.05,
):
    ...
    robust_difference : float
        How much difference between robust and regular fitting do we allow?
    """
    tests = [
        (check_hydro_enabled, {}),
        ...
        (compare_to_robust_fitting, {"factor": robust_difference}),
    ]
