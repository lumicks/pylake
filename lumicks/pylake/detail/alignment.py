...
def align_force_simple(fd_curves, distance_range=1):
    ...
    def get_offset(fd):
        force, distance = fd.sliced(distance_max=np.min(fd.d.data[fd.d.data > 0]) + distance_range)
...
def align_distance_simple(fd_curves, distance_range=1):
    ...
    def linear_fit(fd):
        force, distance = fd.sliced(distance_min=np.max(fd.d.data) - distance_range)
