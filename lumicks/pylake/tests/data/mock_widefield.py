...
def _make_base_description(version, bit_depth):
    ...
    def laser_on(c):
        return f"{c} Excitation Laser on"

    def laser_wavelength(c):
        return f"{c} Excitation Laser wavelength (nm)"

    def laser_power(c, suff):
        return f"{c} Excitation Laser power {suff}(%)"
...
def make_wt_description(version, bit_depth, m_red, m_blue, offsets):
    if version == 1:
        ...
        def alignment_matrices(color):
            return f"Alignment {color} channel"

        def wavelength_fields(color):
            return f"Channel {color} detection wavelength (nm)"

        channel_choices = ("red", "green", "blue")
    else:

        def alignment_matrices(index):
            return f"Channel {index} alignment"

        def wavelength_fields(index):
            return f"Channel {index} detection wavelength (nm)"
...
def write_tiff_file(image, description, n_frames, filename, metadata=True, **kwargs):
    ...
    with tifffile.TiffWriter(filename) as tif:
        for n, frame in enumerate(movie):
            ...
            tif.write(
                ...
                metadata=None,
                contiguous=False,
                extratags=[tag_orientation, tag_datetime] if metadata else [tag_orientation],
                **kwargs,
            )
