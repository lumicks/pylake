...
def _exponential_mixture_log_likelihood_jacobian(params, t, t_min, t_max, t_step=None):
    ...
    def safe_divide(arr, denominator):
        ...
        get numerical issues with divisions by zero. Note that we are _not_ masking by the
        denominator here, but by the nominator. We do not want to silence any divisions of non-zero
        nominator values by zero which are invalid and should be caught."""
        for row in arr:
            mask = abs(row) > 1e-120
            row[mask] /= denominator[mask]
