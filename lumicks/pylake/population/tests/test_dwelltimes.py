...
def test_invalid_bootstrap(exponential_data):
    ...
    with pytest.raises(
        ValueError, match="Number of parameters should be the same as the number of components"
    ):
        DwelltimeBootstrap(fit, np.zeros((3, 1)), np.zeros((3, 1)))

    with pytest.raises(
        ValueError,
        match="Number of amplitude samples should be the same as number of lifetime samples"
    ):
        DwelltimeBootstrap(fit, np.zeros((3, 2)), np.zeros((2, 1)))
