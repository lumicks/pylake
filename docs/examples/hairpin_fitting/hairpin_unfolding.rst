...
If the force offset for the fd curve is different as compared to the offset for the baseline, it can be :ref:`included in the model <model_composition>`.
For this experiment, the same offset that was present in the baseline, is also present in the fd curve. Therefore, we subtract it here::
...
The force-distance curve using the piezo distance should have roughly the same shape as the force-distance curve using the camera tracking distance as plotted above.
In practise there will be small differences, because they are computed from different data sources.
...
a very large value for `dna_ss_hairpin/St`::
    ...
    fit["dna_ss_hairpin/St"].upper_bound = 100000 
    fit["dna_ss_hairpin/St"].value = 100000  # in pN
