...
This Notebook requires the `peakutils` package. Install `peakutils` from the comman line and run the following cell to import it:
...
Below, for example, the green profile is plotted and the coordinates are converted to base pairs using the function `um_to_kbp`. 
The target sites (with kbp coordinates) are plotted as vertical lines.::
...
Below, the coordinates of the markers and target sites are provided in base pairs, and then overlaid with the kymograph using the function `kbp_to_um`::
...
Using the coordinates of the red markers and the function `um_to_kbp` mirometer coordinates can be converted to base pairs and the target binding can be quantified.

There are various ways to quantify target binding, for example [1]_:
