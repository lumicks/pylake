...
For this we want to use an inverted worm-like chain model with a force offset.

While it is possible (and equivalent) to generate this model via `lk.odijk("DNA").invert()`, an optimized inverted WLC
model is explicitly included as a separate model. When only a single WLC is needed, it is best to use this one, as it is
considerably faster. We also include an estimated distance offset::
...
That means we'll have to crop out the section of the data that's outside this range. We can do this by creating a
logical mask which is true for the data we wish to include and false for the data we wish to exclude.
...
Lp and stiffness St for this data set::
...
We can see some differences in the estimates but nothing that would be cause for immediate concern, so let's stick with
the Odijk model for the rest of this analysis as it fits slightly better. One thing we noticed when acquiring the data
was that some of the experiments showed some dynamics. It would be interesting to look at the contour length changes for
these experiments. To this end, we take the model we just fitted and determine a contour length per data point of this
model while keeping all other parameters the same.
