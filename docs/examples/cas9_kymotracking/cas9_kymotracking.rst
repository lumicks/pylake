...
Here we extract the data corresponding to the green channel and downsample the image by a factor of `2`::

    green_channel = kymo.get_channel("green").downsampled_by(2)
...
bead. This binding should be omitted from the analysis::
    ...
    kymowidget = lk.KymoWidgetGreedy(green_channel, axis_aspect_ratio=2, min_length=4, pixel_threshold=3, window=6, sigma=1.4, vmax=8)
...
the longest line we found, and have a look at its position over time::
    ...
    plt.plot(np.array(longest_line.time), np.array(longest_line.coordinate))
    plt.xlabel('Time [s]')
...
around the line from -3 to 3, we can achieve this by::
    ...
    plt.figure()
    plt.plot(longest_line.time, longest_line.sample_from_image(3))
...
segments are when we compare them to the force::
    ...
    ax2 = ax1.twinx()
    line_start_times = np.array([line.time[0] for line in kymowidget.lines])
    line_stop_times = np.array([line.time[-1] for line in kymowidget.lines])
