...
In order to redo the Bluelake calibration, the force data that was used for calibration has to be included in the h5 file.
Note that this force data is not exported nor marked by default; it has to be explicitly added to the exported file.
...
This returns a dictionary with exactly those parameters you would need to reproduce this calibration in a format that :func:`~lumicks.pylake.calibrate_force` will accept.
...
.. note::
    ...
    this means that you are dealing with a pre-calibrated diode. In this case, remember that the values
...
To do so, take the slice and multiply it by the ratio of the old and new calibration factors::
    ...
    recalibrated_force1x = f.force1x * recalibrated_hyco.force_sensitivity / old_calibration.force_sensitivity
